odoo.define('gws_google_maps.MapRenderer', function (require) {
    'use strict';
    ...
    var ICON_URL = '/gws_google_maps/static/src/img/markers/';
    var MARKER_COLORS = [
        'green', 'yellow', 'blue', 'light-green',
        'red', 'magenta', 'black', 'purple', 'orange',
        'pink', 'grey', 'brown', 'cyan', 'white', 'marker_seba', 'marker_pointorange'
    ];
    ...
    var MapRenderer = BasicRenderer.extend({
        ...
        _initLibraryProperties: function (params) {
            if (this.mapLibrary === 'drawing') {
                ...
                this.markers = [];
                this.iconUrl = '/gws_google_maps/static/src/img/markers/';
        ...
        _initMarkerCluster: function () {
            this.markerCluster = new MarkerClusterer(this.gmap, [], {
                imagePath: '/gws_google_maps/static/lib/markercluster/img/m',
                gridSize: 20,
                maxZoom: 17
            });
        },
        ...
        _createWaypoint: function () {
            ...
            var stateData = self.state.data;
            _.each(stateData, function (record, index) {
                latLng = new google.maps.LatLng(record.data[self.fieldLat], record.data[self.fieldLng]);
                counter++;

                // Remove 0x0 markers
                if (latLng.lat() > 0 || latLng.lng() > 0) {
                    var directionsService = self.directionsService;

                    if (index === counter) {
                        options.origin = latLng;
                        counter = -1;
                    }else if (index === stateData.length - 1) {
                        options.destination = latLng;
                    }else {
                        if (!options.waypoints) options.waypoints = [];
                    ...
                    var directionsDisplay = self.directionsDisplay;
