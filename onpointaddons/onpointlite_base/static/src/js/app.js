var app = new Framework7({
  ...
  routes: [
    ...
    },
    {
        ...
        url: '/mobile/logger/detail/{{logger_id}}/',
        options: {
            animate: true,
    ...
    {
        name: 'logger_channel_consumption',
        path: '/logger/channel/consumption/:logger_id/:channel_id',
        url: '/mobile/logger/channel/consumption/{{logger_id}}/{{channel_id}}/',
        options: {
            animate: true,
            transition: 'f7-parallax'
        }
    ...
    },
    {
        ...
        url: '/mobile/logger/channel/form/{{channel_id}}/',
        options: {
            animate: true,
            transition: 'f7-parallax'
        }
...
$ptrContent.on('ptr:refresh', function(e) {
    ...
    // mainView.router.navigate('/mobile/index');
...
$$(document).on('page:init', '.page[data-name="dashboard_page"]', function(e) {
    console.log("masuk..");
...
$$(document).on('page:init', '.page[data-name="logger_detail_page"]', function(e) {
    ...
    form_values['logger_id'] = parseInt($$('#logger_id').val());
    ...
    app.request.postJSON('/mobile/logger/chart', form_values, function(data) {
        data = data['result'];
        var yAxis = data['response']['yAxis'];
        var series = data['response']['series'];
        state_battery = data['response']['state_battery']['src'];
        state_battery_value = data['response']['state_battery']['last_value'];
        state_signal = data['response']['state_signal']['src'];
        state_signal_value = data['response']['state_signal']['last_value'];
        state_temperature = data['response']['state_temperature']['src'];
        state_temperature_value = data['response']['state_temperature']['last_value'];
    ...
    $$('#icon-download').on('click', function(){
        ...
        app.request.postJSON('./logger/report', form_values, function(data) {
            ...
            var url = '/web/content/' + result['response'];
...
$$(document).on('page:init', '.page[data-name="logger_form_page"]', function(e) {
    ...
    $$('.convert-form-to-data').on('click', function () {
        ...
        navigator.geolocation.getCurrentPosition(function onWeatherSuccess(position) {

            form_values['latitude'] = position.coords.latitude;
            form_values['longitude'] = position.coords.longitude;
            ...
            app.request.postJSON('./logger/create', form_values, function(data) {
                ...
                mainView.router.navigate({
                    name: 'logger_detail',
                    params: { logger_id: result.response[0]}},
                    {
                        ignoreCache: true,
                        force: true
                    });
...
function get_logger_channel_consumption_chart(interval) {
    form_values = new Object();
    form_values['logger_id'] = $$('#logger_id').val();
    form_values['channel_id'] = $$('#channel_id').val();
    ...
    app.request.postJSON('/mobile/logger/channel/consumption/chart', form_values, function(data) {
        data = data['result'];
        var title = '';
        var yAxis = data['response']['yAxis'];
        var series = data['response']['series'];
        $$('#report_period').val(data['response']['report_period']);
...
$$(document).on('page:init', '.page[data-name="logger_channel_consumption_page"]', function(e) {
    get_logger_channel_consumption_chart('default');
    var chart_image_url = '';
    ...
    $$('.btn_consumption_intervals').on('click', function(e) {
        ...
        chart_image_url = get_logger_channel_consumption_chart(interval);
    });
