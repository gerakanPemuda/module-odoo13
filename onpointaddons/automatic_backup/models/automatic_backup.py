...
class AutomaticBackup(models.Model):
    ...
    limit_time_real = fields.Integer(string='Maximum allowed Real time per request (in seconds)',
                                     compute='compute_odoo_settings', inverse='set_odoo_settings')
    limit_time_real_cron = fields.Integer(string='Maximum allowed Real time per cron job (in seconds / Set to 0 for no limit)',
                                          compute='compute_odoo_settings', inverse='set_odoo_settings')
...
class Cron(models.Model):
    ...
    def get_default_nextcall(self):
        return datetime.now() + timedelta(hours=1)

    nextcall = fields.Datetime(default=lambda self: self.get_default_nextcall())
    ...
    dropbox_flow = fields.Many2one('ir.attachment')
    ...
    def unlink(self):
        for line in self:
            if line.dropbox_flow:
                line.dropbox_flow.unlink()
    ...
    @api.constrains('backup_destination', 'dropbox_authorization_code')
    ...
    def create_backup(self, check=False):
        ...
        backup_path = filestore + os.sep + 'backup_tmp'
        if os.path.exists(backup_path):
            os.remove(backup_path)
        backup_binary = open(backup_path, 'wb+')
        if check is False:
            odoo.service.db.dump_db(self.env.cr.dbname, backup_binary, self.backup_type)
        else:
...
class IrAttachment(models.Model):
    ...
    full_path = fields.Char(compute='compute_full_path', store=False)

    def compute_full_path(self):
        for attachment in self:
            filestore = odoo.tools.config.filestore(self.env.cr.dbname)
            attachment.full_path = os.path.join(filestore, attachment.store_fname)
